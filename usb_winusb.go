...
func (e CDWORD) Error() string {
	return fmt.Sprintf("Windows Error: 0x%x", uint(e))
}

const (
	SUCCESS                 CDWORD = C.ERROR_SUCCESS
	ERROR_INVALID_STATE     CDWORD = C.ERROR_INVALID_STATE
	ERROR_INVALID_HANDLE    CDWORD = C.ERROR_INVALID_HANDLE
	ERROR_INVALID_PARAMETER CDWORD = C.ERROR_INVALID_PARAMETER
	ERROR_OUTOFMEMORY       CDWORD = C.ERROR_OUTOFMEMORY
	ERROR_GEN_FAILURE       CDWORD = C.ERROR_GEN_FAILURE
	ERROR_OBJECT_NOT_FOUND  CDWORD = C.ERROR_OBJECT_NOT_FOUND
	ERROR_NOT_SUPPORTED     CDWORD = C.ERROR_NOT_SUPPORTED
	ERROR_SHARING_VIOLATION CDWORD = C.ERROR_SHARING_VIOLATION
	ERROR_BAD_COMMAND       CDWORD = C.ERROR_BAD_COMMAND
)
